...
double GetBeam_pol(int event,int pol) {
  ...
  if ( fRandom->Rndm() < 0.5 ) {
    ...
    beam_pol = double(pol) * (mean_beampol + TMath::Abs(beam_pol));
  }
  else {
    sigma_beampol = 0.025;
    beam_pol = fRandom->Gaus(0.0, sigma_beampol);
...
void Run(){
  ...
  TFile *file_out = new TFile("output_test_polasym_10000.root","RECREATE");
  ...
  TH1F *beampol_h = new TH1F("beampol_h","Beam Polarization; Pol",100,0.,1.0);
